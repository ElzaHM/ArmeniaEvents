...
function resolveUserRole(user: User): UserRole {
  const fromUserMetadata = parseUserRole(user.user_metadata as Record<string, unknown> | undefined);
  if (fromUserMetadata !== 'user') {
    return fromUserMetadata;
  }

  return parseUserRole(user.app_metadata);
}
